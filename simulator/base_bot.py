...
class OrderDecision:
    ...
    ticker:        str | None   # None if HOLD
    quantity:      int | None
    limit_price:   float | None
    reasoning:     str
    headline_used: str | None   # the headline that triggered this decision
...
class BaseBot(ABC):
    ...
    def positions(self) -> dict:
        ...
        if self.llm_provider == "claude":
            ...
            raise ValueError(f"Unknown llm_provider '{llm_provider}'. Use 'claude' or 'openai'.")
    ...
    def _build_prompt(self, context: dict) -> str:
        ...
        positions_str = (
            "\n".join(f"  {ticker}: {qty} shares" for ticker, qty in context["positions"].items())
            or "  (none)"
        )
...
{_JSON_FORMAT_INSTRUCTIONS}"""
    ...
    def _call_llm(self, prompt: str) -> dict:
        ...
        WHY: Any failure — API error, network timeout, bad JSON, missing fields —
        returns HOLD. An automated trading system must never crash on an LLM error.
        """
        try:
            if self.llm_provider == "claude":
            ...
            required = {"action", "ticker", "quantity", "limit_price", "reasoning", "headline_used"}
